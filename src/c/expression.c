...
bool
is_not_zero_constant_expr(const struct expression *expr)
{
...
expr_map(const struct expression *fun, const struct expression *array)
{
    ...
    int trigger_error =
	(fun->type != type_generic
	 && array->type != type_generic);
		
    if ( trigger_error && !type_is_function( fun->type ) )
    {
	error("first operand to map operator"
	      " must be a function.\n");
    }
    else
    {
    ...
    {
	error("second operand to map operator"
	      " must be an array.\n");
    }
    else if ( trigger_error )
    {
...
const struct expression *
expr_reduce(const struct expression *fun, const struct expression *array)
{
    bool valid_op_F = false, valid_op_A = false;
    ...
    {
	error("First operand to reduce operator must be"
	      " a function.\n");
    }
    else
    {
...
static const struct expression *
operation(enum expression_type et, const char *opname,
	  const struct expression *lop, const struct expression *rop)
{
    struct expression *expr = expr_new( et );
...
const struct expression*
expr_fpsicast(const struct expression *op,
	      const struct type *target)
{
    struct expression *e;
...
const struct expression *
expr_cast(const struct expression *op, const struct type* target)
{
...
const struct expression *
expr_bitcast(const struct expression *op, const struct type* target)
{
    struct expression *e;
...
const struct expression *
expr_sign_extend(const struct expression *op, const struct type *target)
{
    struct expression *e;
...
const struct expression *
expr_zero_extend(const struct expression *op, const struct type *target)
{
    struct expression *e;
    assert( type_is_integer( target ) );
...
const struct expression *
expr_trunc(const struct expression *op, const struct type *target)
{
    struct expression *e;
    assert( type_is_integer( target ) );
...
const struct expression *
expr_array_size(const struct expression *array)
{
