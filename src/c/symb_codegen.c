...
void symb_cg(struct symbol *sy)
{
	...
	asprintf(&init_code,
		 ...
		 typestr,
		 sy->type->type == TYPE_ARRAY?", align 8": "");
...
st_array_alloc_rec(const struct expression *array, int depth)
{
	...
	const struct expression *mul = expr_multiplication(
		type->array_type.array_size,
	...
	if ( type_is_array(type_array_values(type)) 
	     && is_not_zero_constant_expr(
		     type_array_size(type_array_values(type))))
	{
		char *name;
