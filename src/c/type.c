			     ...
			     const struct expression *array_size)
{
	...
	ty->array_type.values =	type_new_array_type(
		type_array_values(values),
		array_size
	);
	return ty;
}
...
static 
const char *str_expression_size(const struct expression *expr)
{
	char *str = "";
...
{
	return ( type->type == TYPE_BYTE || type->type == TYPE_SHORT ||
...
{
	return ( type->type == TYPE_BYTE || type->type == TYPE_SHORT ||
		 type->type == TYPE_INT ||  type->type == TYPE_LONG ) ;
}
